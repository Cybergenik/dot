...
beautiful.init("/home/luciano/.config/awesome/themes/xresources/theme.lua")
...
-- {{{ Centered run prompt
local centered_prompt_wibox = wibox({
    visible = false,
    ontop = true,
    width = 400,
    height = 30,
...
local centered_prompt_widget = wibox.widget.textbox()

centered_prompt_wibox:setup {
    {
        centered_prompt_widget,
        left = 10,
        right = 10,
...
local function show_centered_prompt()
    local s = awful.screen.focused()
    centered_prompt_wibox.screen = s
    centered_prompt_wibox.x = s.geometry.x + math.floor((s.geometry.width - centered_prompt_wibox.width) / 2)
    centered_prompt_wibox.y = s.geometry.y + math.floor((s.geometry.height - centered_prompt_wibox.height) / 2)
    centered_prompt_wibox.visible = true
    awful.prompt.run {
        prompt = "<b>Run: </b>",
        textbox = centered_prompt_widget,
        done_callback = function()
            centered_prompt_wibox.visible = false
        end,
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey,           }, "space", function () awful.spawn.with_shell("rofi-claude") end,
              {description = "ask claude", group = "launcher"}),
...
clientkeys = awful.util.table.join(
    ...
    awful.key({ modkey, }, "a", 
        function () awful.spawn.easy_async_with_shell("ra -x -e",
            function (stdout) naughty.notify { text = "audio: " .. stdout } end)
        end,
        {description = "audio", group = "record"}),
    awful.key({ modkey, "Shift" }, "a", 
        function () awful.spawn.easy_async_with_shell("ra -p -x -e", 
            function (stdout) naughty.notify { text = "audio: " .. stdout } end)
        end,
        {description = "audio vlc", group = "record"}),
    awful.key({ modkey, "Control" }, "a",
        function () awful.spawn.easy_async_with_shell("ra -u -x -e",
            function (stdout) naughty.notify { text = "audio: " .. stdout } end)
        end,
        {description = "audio upload", group = "record"}),
    awful.key({ modkey, "Control", "Shift" }, "a",
        function () awful.spawn.easy_async_with_shell("ra -u -p -x -e",
            function (stdout) naughty.notify { text = "audio: " .. stdout } end)
        end,
        {description = "audio upload vlc", group = "record"}),
    awful.key({ "Mod1" }, "a",
        function ()
            awful.prompt.run {
                prompt       = "audio: ",
                textbox      = awful.screen.focused().mypromptbox.widget,
                exe_callback = function (input) awful.spawn.easy_async_with_shell("ra -x -e -n " .. input,
                    function (stdout) naughty.notify { text = "audio: " .. stdout } end)
                end,
                history_path = awful.util.get_cache_dir() .. "/history_audio"
            }
        end,
        {description = "audio named", group = "record"}),
    awful.key({ "Mod1", "Shift" }, "a",
        function ()
            awful.prompt.run {
                prompt       = "audio: ",
                textbox      = awful.screen.focused().mypromptbox.widget,
                exe_callback = function (input) awful.spawn.easy_async_with_shell("ra -p -x -e -n " .. input,
                    function (stdout) naughty.notify { text = "audio: " .. stdout } end)
                end,
                history_path = awful.util.get_cache_dir() .. "/history_audio"
            }
        end,
        {description = "audio named feh", group = "record"}),
    awful.key({ "Mod1", "Control" }, "a",
        function ()
            awful.prompt.run {
                prompt       = "audio: ",
                textbox      = awful.screen.focused().mypromptbox.widget,
                exe_callback = function (input) awful.spawn.easy_async_with_shell("ra -u -x -e -n " .. input,
                    function (stdout) naughty.notify { text = "audio: " .. stdout } end)
                end,
                history_path = awful.util.get_cache_dir() .. "/history_audio"
            }
        end,
        {description = "audio named upload", group = "record"}),
    awful.key({ "Mod1", "Control", "Shift" }, "a",
        function ()
            awful.prompt.run {
                prompt       = "audio: ",
                textbox      = awful.screen.focused().mypromptbox.widget,
                exe_callback = function (input) awful.spawn.easy_async_with_shell("ra -u -p -x -e -n " .. input,
                    function (stdout) naughty.notify { text = "audio: " .. stdout } end)
                end,
                history_path = awful.util.get_cache_dir() .. "/history_audio"
            }
        end,
        {description = "audio named upload vlc", group = "record"})
)
...
-- Claude popup client keys (Escape to dismiss)
local claude_clientkeys = awful.util.table.join(
    clientkeys,
...
awful.rules.rules = {
    ...
    -- Ghostty: always tile even in floating mode
    { rule = { class = "com.mitchellh.ghostty" },
      properties = { floating = false }
    },

    -- Claude popup
    { rule = { instance = "claude_popup" },
      properties = {
        floating = true,
        ontop = true,
        titlebars_enabled = false,
        keys = claude_clientkeys,
      },
      callback = function(c)
        local s = c.screen
        local geo = s.workarea
        c:geometry({
            width = math.floor(geo.width * 0.6),
            height = math.floor(geo.height * 0.6),
            x = geo.x + math.floor(geo.width * 0.2),
            y = geo.y + math.floor(geo.height * 0.2),
        })
      end
    },
...
client.connect_signal("unfocus", function(c)
    ...
    -- Dismiss claude popup when it loses focus (click away)
    if c.instance == "claude_popup" then
        c:kill()
    end
end)
-- }}}
